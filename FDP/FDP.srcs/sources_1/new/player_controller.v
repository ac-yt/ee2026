...
module player_one_controller (input clk,
...
module player_controller (input clk,
                          ...
                          output [3:0] player_tx, player_ty,
                          output [6:0] player_x,
                          output [5:0] player_y,
                          output player_dead, 
...
//    end
    ...
    movement_controller player_move (.clk(clk), .map_changed(map_changed), .spawn_tx(spawn_tx), .spawn_ty(spawn_ty),
                                     .goal_tx(goal_tx), .goal_ty(goal_ty), .tile_map_flat(tile_map_flat), .speed(speed), .is_player(1),
                                     .next_is_block(next_is_block), .pos_tx_out(player_tx), .pos_ty_out(player_ty), .pos_x(player_x), .pos_y(player_y),
                                     .as_update(update), .as_baw(blocks_as_walls), .path_flat_x(path_flat_x), .path_flat_y(path_flat_y),
                                     .path_valid(path_valid), .path_len(path_len));
